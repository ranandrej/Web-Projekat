...
import { PlusIcon, MinusIcon, ChevronLeftIcon } from '@heroicons/react/24/outline'
...
const EditQuiz = () => {
  ...
  const { updateQuiz, getCategories, getQuizById, loading } = useQuiz()
  ...
  const loadQuiz = async () => {
    ...
    try {
      ...
      if (quiz) {
        // Pre-fill the form with existing quiz data
        reset({
          title: quiz.title,
  ...
  const validateStep2 = async () => {
    ...
    if (!isValid) {
      ...
      values.questions.forEach((question, index) => {
        ...
        if (question.answers && !question.answers.some(answer => answer.isCorrect)) {
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <select
                          {...register(`questions.${questionIndex}.type`, { valueAsNumber: true })}
                          className="input-field"
                        >
